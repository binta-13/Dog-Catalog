...
export const actions = {
    ...
    await this.$axios
      ...
      .then((data) => {
        for (const key in data) {
          this.$axios
            .$get(`/api/breed/${key}/images/random/1`)
            .then((response) => {
              commit("SET_DOG_LIST", {
                name: key,
  ...
  async getInputBreed({ commit }, param) {
    await this.$axios
      .$get(`/api/breed/${param}/images/random`)
      .then((response) => {
